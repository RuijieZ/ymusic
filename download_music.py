...
def read_all_urls(path):
    try:
        with open(path, "r") as f:
            return [line.strip().split(" ")[0] for line in f if line[0] != "#"]
...
def download_video(url, path):
    try:
        cmd = "youtube-dl -i -f mp4 '{}' -o '{}/%(title)s.%(ext)s'".format(url, path)
...
if __name__ == '__main__':
    ...
    processes =[Process(target=download_video, args=(url, args.download_path)) for url in urls]
